    ...
    Select Case KeyCode
        ...
        Case vbKeyF1:
                ...
                frmHelp.Help_Values = Space(1) & vbCrLf & _
                                      "Note: The following are Code File Setup Key Shortcuts." & vbCrLf & _
                                      Space(1) & vbCrLf & _
...
Private Sub Closed_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Call BitBlt(frmCodeFile.Closed.hDC, 0, 0, 73, 50, frmLogin.Source.hDC, 18, 107, SRCCOPY)
    frmCodeFile.Closed.Refresh
End Sub
...
Private Sub Maximized_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Call BitBlt(frmCodeFile.Maximized.hDC, 0, 0, 73, 50, frmLogin.Source.hDC, 18, 72, SRCCOPY)
    frmCodeFile.Maximized.Refresh
End Sub
...
Private Sub Minimized_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Call BitBlt(frmCodeFile.Minimized.hDC, 0, 0, 73, 50, frmLogin.Source.hDC, 18, 124, SRCCOPY)
    frmCodeFile.Minimized.Refresh
End Sub
...
Private Sub Display_Fields()
    On Error Resume Next
    If datprimary.AbsolutePosition <> -1 Then
        txtField(0) = IIf(IsNull(datprimary("CODE_NAME")), "", datprimary("CODE_NAME"))
        txtField(1) = IIf(IsNull(datprimary("CODE_VALUE")), "", datprimary("CODE_VALUE"))
        txtField(2) = IIf(IsNull(datprimary("CODE_DESC")), "", datprimary("CODE_DESC"))
    Else
        Clear_Fields
    End If
End Sub
...
Private Sub Update_Fields(isNew As Boolean)
    ...
    Else
        datprimary.Edit
    End If
        datprimary("CODE_NAME") = txtField(0)
        datprimary("CODE_VALUE") = txtField(1)
        datprimary("CODE_DESC") = txtField(2)
        datprimary.Update
...
Function Get_File_Code() As Boolean
    On Error Resume Next
    strs = "select * from CODE_FILE where CODE_NAME = '" & txtField(0) & "'"
    Set dummy = frmLogin.db.OpenRecordset(strs)
    If dummy.AbsolutePosition <> -1 Then
        Get_File_Code = True
    Else
